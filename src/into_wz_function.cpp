...
static unique_ptr<FunctionData> IntoWzBind(ClientContext &context,
    ...
    for (auto &kv : input.named_parameters) {
        if (kv.first == "secret") {
            if (!kv.second.IsNull()) bind_data->secret_name = StringValue::Get(kv.second);
        } else if (kv.first == "source_table") {
            if (!kv.second.IsNull()) bind_data->source_table = StringValue::Get(kv.second);
        } else if (kv.first == "gui_verfahren_id") {
            if (!kv.second.IsNull()) bind_data->gui_verfahren_id = StringValue::Get(kv.second);
        } else if (kv.first == "lng_kanzlei_konten_rahmen_id") {
            if (!kv.second.IsNull()) bind_data->lng_kanzlei_konten_rahmen_id = kv.second.GetValue<int64_t>();
            ...
            if (!kv.second.IsNull()) bind_data->skip_duplicate_check = kv.second.GetValue<bool>();
        } else if (kv.first == "skip_fk_check") {
            if (!kv.second.IsNull()) bind_data->skip_fk_check = kv.second.GetValue<bool>();
        }
    }
...
static void BatchIntoWzExecute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
    ...
    case BatchExecutionPhase::VALIDATE_PARAMS: {
        ...
        if (!IsValidSqlIdentifier(bind_data.secret_name)) {
            BatchCleanupAndError(state, bind_data, output, "ERROR",
                "Invalid secret name: must contain only alphanumeric characters, underscores, and dots");
            return;
        }
    ...
    case BatchExecutionPhase::VALIDATE_GROUPS: {
        ...
        if (!bind_data.skip_fk_check) {
            ...
            if (!fk_valid) {
                BatchCleanupAndError(state, bind_data, output, "ERROR",
                    "FK validation failed: " + error_msg);
                return;
            }
        }
